...
def make_request(method, base_url, path, headers=None, body=None, timeout=30, use_proxy=True, retries=2):
    url = base_url + path
    request_headers = {
        'Accept': '*/*',
    ...
    for attempt in range(retries + 1):
        try:
            if method == 'GET':
                resp = requests.get(url, headers=request_headers, timeout=timeout, proxies=proxies)
            elif method == 'POST':
                resp = requests.post(url, headers=request_headers, data=body_bytes, timeout=timeout, proxies=proxies)
            else:
